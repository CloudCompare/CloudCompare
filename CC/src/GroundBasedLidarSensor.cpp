...
inline void GroundBasedLidarSensor::projectPoint(const CCVector3& sourcePoint, CCVector2& destPoint, ScalarType &dist)
{
...
SimpleCloud* GroundBasedLidarSensor::project(GenericCloud* theCloud, int& errorCode, bool getParameters)
{
	...
	if (dB.zBuff)
		delete[] dB.zBuff;
	dB.zBuff = new ScalarType[zBuffSize];
...
int GroundBasedLidarSensor::fillZBufferHoles()
{
	...
	ScalarType *_zBuffTemp = zBuffTemp+dx+1; //on se positionne sur la deuxième ligne, deuxième colonne du buffer temporaire
	ScalarType *_zBuff = dB.zBuff; //on se positionne sur la première ligne du vrai buffer
	for (y=0;y<dB.h_buff;++y)
	{
		//on recopie chaque ligne à "l'intérieur" du buffer
		memcpy(_zBuffTemp,_zBuff,dB.l_buff*sizeof(ScalarType));
		_zBuffTemp += dx;
		_zBuff += dB.l_buff;
	}
			...
			if (_zBuffTemp[1]==0) //si c'est un trou !
			{
				...
				if (nsup>3)
				{
					...
					dB.zBuff[x+y*dB.l_buff] = (zu[0]+zu[1]+zu[2]+
						...
						zd[0]+zd[1]+zd[2])/ScalarType(nsup);
				}
			}
...
uchar GroundBasedLidarSensor::checkVisibility(const CCVector3& P)
{
	...
	//s'il est hors-champ
	if (x<0 || x>=dB.l_buff || y<0 || y>=dB.h_buff)
...
ScalarType GroundBasedLidarSensor::getSensorRange()
{
	return sensorRange;
}

void GroundBasedLidarSensor::setSensorRange(ScalarType range)
{
	sensorRange = range;
}
...
ScalarType GroundBasedLidarSensor::getUncertainty()
{
	return uncertainty;
}

void GroundBasedLidarSensor::setUncertainty(ScalarType u)
{
	uncertainty=u;
}
