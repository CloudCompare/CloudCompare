...
void ccHObject::setParent(ccHObject* anObject)
{
    m_parent = anObject;
}
...
{
	return (m_children.size()!=0 ? m_children.front() : 0);
}

ccHObject* ccHObject::getLastChild() const
{
    return (m_children.size()!=0 ? m_children.back() : 0);
}
...
void ccHObject::drawMeOnly(CC_DRAW_CONTEXT& context)
{
}
