...
struct MLSParameters
{
    ...
    MLSParameters()
        : order_(0), polynomial_fit_(false), search_radius_(0),
          sqr_gauss_param_(0), compute_normals_(false), upsample_method_(NONE),
          upsampling_radius_(0), upsampling_step_(0), step_point_density_(0),
          dilation_voxel_size_(0), dilation_iterations_(0)
    {
    }
...
* if only the point cloud is given PCL default parameters are used (that are not
* really good, so please give parameters)
*  \note that we can pass different types as input for this function:
* - PointXYZI
* - PointNormal
* - PointXYZRGB
* \note if a PointType with a scale field is passed as output type, scales will
* be returned together with the return cloud
*/
...
int estimateSIFT(const typename pcl::PointCloud<PointInT>::Ptr in_cloud,
                 ...
                 int nr_scales = 0, float min_scale = 0,
...
template <typename PointInT, typename PointOutT>
int computeIntensitySPINImages(
    const typename pcl::PointCloud<PointInT>::Ptr incloud, const float radius,
    const int k_nn,
    const bool useKnn, // true if use knn, false if radius search
    const int n_distance_bins, const int n_intensity_bins,
...
int removeOutliersStatistical(const PCLCloud::ConstPtr incloud, const int &k,
                              const float &nStds, PCLCloud::Ptr outcloud);
...
**/
void copyScalarFields(const ccPointCloud *inCloud, ccPointCloud *outCloud,
                      pcl::PointIndicesPtr &in2outMapping,
                      bool overwrite = true);

void copyRGBColors(const ccPointCloud *inCloud, ccPointCloud *outCloud,
                   pcl::PointIndicesPtr &in2outMapping, bool overwrite = true);
