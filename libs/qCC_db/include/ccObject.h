...
class QCC_DB_LIB_API ccObject : public ccSerializableObject
{
  public:
	...
	**/
	...
	virtual inline unsigned getUniqueID() const
	{
		return m_uniqueID;
	}
	...
	**/
	...
	virtual inline void toggleActivation()
	{
	...
	{
		return (getClassID() & CC_LEAF_BIT) != 0;
	}
	inline bool isCustom() const
	{
		return (getClassID() & CC_CUSTOM_BIT) != 0;
	}
	...
	inline bool isKindOf(CC_CLASS_ENUM type) const
	{
  ...
  protected:
	//! Returns flag state
	virtual inline bool getFlagState(CC_OBJECT_FLAG flag) const
	{
		return (m_flags & flag);
	}
