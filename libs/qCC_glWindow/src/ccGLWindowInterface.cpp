	...
	if (!m_initialized)
	{
		...
		if (glContext->hasExtension(QByteArrayLiteral("GL_KHR_debug")))
		{
			...
			QObject::connect(logger, &QOpenGLDebugLogger::messageLogged, [&](const QOpenGLDebugMessage& debugMessage)
			                 { HandleLoggedMessage(debugMessage, getUniqueID()); });
...
void ccGLWindowInterface::renderText(int x, int y, const QString& str, uint16_t uniqueID /*=0*/, const QFont& font /*=QFont()*/)
{
	...
	// and then we convert this QImage to a texture!
	{
		...
		glFunc->glLoadIdentity();
		{
			...
			glFunc->glTexCoord2f(0, ratioH);
			glFunc->glVertex3i(0, 0, 0);
			glFunc->glTexCoord2f(ratioW, ratioH);
			glFunc->glVertex3i(textRect.width(), 0, 0);
			glFunc->glTexCoord2f(ratioW, 0);
			glFunc->glVertex3i(textRect.width(), textRect.height(), 0);
			glFunc->glTexCoord2f(0, 0);
			glFunc->glVertex3i(0, textRect.height(), 0);
			glFunc->glEnd();
...
void ccGLWindowInterface::startFrameRateTest()
{
	...
	QObject::connect(&s_frameRateTimer, &QTimer::timeout, asQObject(), [=]()
	                 { redraw(); },
	                 Qt::QueuedConnection);
...
void ccGLWindowInterface::updateFrameRateTest()
{
	if (s_frameRateTestInProgress)
	{
		...
		{
			QTimer::singleShot(0, [&]()
			                   { ccGLWindowInterface::stopFrameRateTest(); });
		}
		else
		{
	...
	else // no frame rate test in progress
	{
		...
		if (m_currentLODState.inProgress)
		{
			if ((!m_LODPendingRefresh || m_LODPendingIgnore) && !m_mouseMoved && !m_mouseButtonPressed)
			{
				...
				QTimer::singleShot(std::max(baseLODRefreshTime_ms - displayTime_ms, Q_INT64_C(0)), [&]()
				                   { renderNextLODLevel(); });
			}
		}
		else
