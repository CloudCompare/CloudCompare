...
CC_FILE_ERROR SinusxFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	//write header
	outFile << "C Generated by CloudCompare" << Qt::endl;
	...
	for (size_t i = 0; i < profiles.size(); ++i)
	{
		...
		//new block
		outFile << "B S" << Qt::endl; //B + curve type (S = set of 3D points)
		outFile << "CN " << poly->getName() << Qt::endl; //CN + name
		outFile << "CP 1 " << (poly->isClosed() ? 1 : 0) << Qt::endl; //CP + isLinked + isClosed
		outFile << "CP " << (upDir == 2 ? 0 : (upDir == 1 ? 2 : 1)) << Qt::endl; //base plane: 0 = (XY), 1 = (YZ), 2 = (ZX)
