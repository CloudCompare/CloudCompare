	...
	, m_cloud(cloud)
{
	...
	if (m_cloud)
	{
		...
		interpolateSFCheckBox->setEnabled(cloud->hasScalarFields());
		scalarFieldProjection->setEnabled(cloud->hasScalarFields());
...
{
	if (!interpolateSFCheckBox->isEnabled() || !interpolateSFCheckBox->isChecked())
