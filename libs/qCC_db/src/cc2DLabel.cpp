...
//'Delta' character
static constexpr QChar MathSymbolDelta(0x0394);

static const QString         CENTER_STRING  = QObject::tr("Center");
static constexpr const char* POINT_INDEX_0  = "pi0";
static constexpr const char* POINT_INDEX_1  = "pi1";
static constexpr const char* POINT_INDEX_2  = "pi2";
static constexpr const char* ENTITY_INDEX_0 = "ei0";
static constexpr const char* ENTITY_INDEX_1 = "ei1";
static constexpr const char* ENTITY_INDEX_2 = "ei2";
...
void AddPointCoordinates(QStringList& body, QString pointShortName, const CCVector3& P, const ccShiftedObject& shiftedObject, int precision)
{
	...
	{
		body << pointShortName;
		pointShortName = QString("  [shifted]");
	}

	pointShortName += QString(" (%1;%2;%3)").arg(P.x, 0, 'f', precision).arg(P.y, 0, 'f', precision).arg(P.z, 0, 'f', precision);
	body << pointShortName;
...
void AddPointCoordinates(QStringList& body, const cc2DLabel::PickedPoint& pp, int precision, const QString& pointName = QString())
{
	QString                pointShortName;
	const ccShiftedObject* shiftedObject = nullptr;
	...
	{
		const ccGenericPointCloud* vertices = pp._mesh->getAssociatedCloud();
		if (!vertices)
		{
			assert(false);
			return;
...
void cc2DLabel::getLabelInfo1(LabelInfo1& info) const
{
	...
	if (!pp.entityCenterPoint)
	{
		...
		if (pp._cloud)
		{
			...
			if (pp._cloud->isA(CC_TYPES::POINT_CLOUD))
			{
				const ccPointCloud* pc      = static_cast<ccPointCloud*>(pp._cloud);
		...
		{
			const CCVector3d w(pp.uv, 1.0 - pp.uv.x - pp.uv.y);
			// normal
			...
			if (info.hasSF)
			{
				const CCCoreLib::VerticesIndexes* vi = pp._mesh->getTriangleVertIndexes(pp.index);
				assert(vi);
				...
				const ccScalarField* sf = nullptr;

				// fetch the real scalar field if possible
				if (vertices->isA(CC_TYPES::POINT_CLOUD))
				{
					sf = static_cast<const ccPointCloud*>(vertices)->getCurrentDisplayedScalarField();
				}
				...
				if (vertices->isA(CC_TYPES::POINT_CLOUD))
				{
					const ccPointCloud* pc      = static_cast<const ccPointCloud*>(vertices);
					unsigned            sfCount = pc->getNumberOfScalarFields();
					for (unsigned i = 0; i < sfCount; ++i)
					{
...
void cc2DLabel::getLabelInfo2(LabelInfo2& info) const
{
	...
	// 1st point
	const CCVector3 P1 = m_pickedPoints[0].getPointPosition();
	// 2nd point
	const CCVector3 P2 = m_pickedPoints[1].getPointPosition();
...
void cc2DLabel::getLabelInfo3(LabelInfo3& info) const
{
	...
	const CCVector3 P2 = m_pickedPoints[1].getPointPosition();
	// 3rd point
	const CCVector3 P3 = m_pickedPoints[2].getPointPosition();

	// area
	const CCVector3 P1P2 = P2 - P1;
	const CCVector3 P1P3 = P3 - P1;
	const CCVector3 P2P3 = P3 - P2;
...
QStringList cc2DLabel::getLabelContent(int precision) const
{
	...
	case 3: // triangle/plane
	{
		...
		// area
		const QString areaStr = QString("Area = %1").arg(info.area, 0, 'f', precision);
		body << areaStr;
		...
		// normal
		const QString normStr = QString("Normal: (%1;%2;%3)").arg(info.normal.x, 0, 'f', precision).arg(info.normal.y, 0, 'f', precision).arg(info.normal.z, 0, 'f', precision);
		body << normStr;
...
void cc2DLabel::drawMeOnly3D(CC_DRAW_CONTEXT& context)
{
	...
	if (!entityPickingMode)
	{
		for (size_t i = 0; i < count; i++)
		{
			...
			const CCVector3 P3D = m_pickedPoints[i].getPointPosition();
	...
	case 1:
	{
		// display point marker as spheres
		{
			...
			for (size_t i = 0; i < count; i++)
			{
				...
				glFunc->glPushMatrix();
				const CCVector3 P = m_pickedPoints[i].getPointPosition();
				...
				scale = scale * context.devicePixelRatio;
...
// display parameters
static constexpr int c_margin        = 5;
static constexpr int c_tabMarginX    = 5;
static constexpr int c_tabMarginY    = 2;
static constexpr int c_arrowBaseSize = 3;
// static const int c_buttonSize = 10;

static constexpr ccColor::Rgba c_darkGreen(0, 200, 0, 255);
...
struct Tab
{
	 ...
	 **/
	inline void setMaxBlockPerRow(int maxBlock)
	{
		maxBlockPerRow = maxBlock;
	}
...
void cc2DLabel::drawMeOnly2D(CC_DRAW_CONTEXT& context)
{
	...
	// in 2D so that they always appear above the entities
	{
		...
		if (visibleCount)
		{
			...
			{
				static constexpr QChar ABC[3]{'A', 'B', 'C'};
	...
	const QString title     = getTitle(precision);
			...
			if (m_showFullBody)
			{
				...
				try
				{
					if (count == 1)
					{
						...
						const CCVector3 P         = m_pickedPoints[0].getPointPosition();
						...
						{
							int              c  = tab.add2x3Block();
							const CCVector3d Pd = cloud->toGlobal3d(P);
							tab.colContent[c] << "Xg";
						...
						if (info.hasNormal)
						{
							int c = tab.add2x3Block();
							tab.colContent[c] << "Nx";
							tab.colContent[c + 1] << QString::number(info.normal.x, 'f', precision);
							tab.colContent[c] << "Ny";
							tab.colContent[c + 1] << QString::number(info.normal.y, 'f', precision);
							tab.colContent[c] << "Nz";
							tab.colContent[c + 1] << QString::number(info.normal.z, 'f', precision);
						}
						...
						if (info.hasRGB)
						{
							int c = tab.add2x3Block();
							tab.colContent[c] << "R";
							tab.colContent[c + 1] << QString::number(info.color.r);
							tab.colContent[c] << "G";
							tab.colContent[c + 1] << QString::number(info.color.g);
							tab.colContent[c] << "B";
							tab.colContent[c + 1] << QString::number(info.color.b);
						}
					}
					else if (count == 2)
					{
						...
						// 1st block: dX, dY, dZ
						{
							...
							tab.colContent[c + 1] << QString::number(info.diff.x, 'f', precision);
							tab.colContent[c] << MathSymbolDelta + QString("Y");
							tab.colContent[c + 1] << QString::number(info.diff.y, 'f', precision);
							tab.colContent[c] << MathSymbolDelta + QString("Z");
							tab.colContent[c + 1] << QString::number(info.diff.z, 'f', precision);
						}
						// 2nd block: dXY, dXZ, dZY
						{
							...
							PointCoordinateType dXZ = sqrt(info.diff.x * info.diff.x + info.diff.z * info.diff.z);
							PointCoordinateType dZY = sqrt(info.diff.z * info.diff.z + info.diff.y * info.diff.y);
							tab.colContent[c] << MathSymbolDelta + QString("XY");
							tab.colContent[c + 1] << QString::number(dXY, 'f', precision);
							tab.colContent[c] << MathSymbolDelta + QString("XZ");
							tab.colContent[c + 1] << QString::number(dXZ, 'f', precision);
							tab.colContent[c] << MathSymbolDelta + QString("ZY");
							tab.colContent[c + 1] << QString::number(dZY, 'f', precision);
						}
					}
					else if (count == 3)
					{
						...
						// next block: indexes
						{
							...
							tab.colContent[c + 1] << (m_pickedPoints[0].itemTitle());
							tab.colContent[c] << "index.B";
							tab.colContent[c + 1] << (m_pickedPoints[1].itemTitle());
							tab.colContent[c] << "index.C";
							tab.colContent[c + 1] << (m_pickedPoints[2].itemTitle());
						}
						// next block: edges length
						{
							int c = tab.add2x3Block();
							tab.colContent[c] << "AB";
							tab.colContent[c + 1] << QString::number(info.edges.u[0], 'f', precision);
							tab.colContent[c] << "BC";
							tab.colContent[c + 1] << QString::number(info.edges.u[1], 'f', precision);
							tab.colContent[c] << "CA";
							tab.colContent[c + 1] << QString::number(info.edges.u[2], 'f', precision);
						}
						// next block: angles
						{
							int c = tab.add2x3Block();
							tab.colContent[c] << "angle.A";
							tab.colContent[c + 1] << QString::number(info.angles.u[0], 'f', precision);
							tab.colContent[c] << "angle.B";
							tab.colContent[c + 1] << QString::number(info.angles.u[1], 'f', precision);
							tab.colContent[c] << "angle.C";
							tab.colContent[c + 1] << QString::number(info.angles.u[2], 'f', precision);
						}
						// next block: normal
						{
							int c = tab.add2x3Block();
                             ...
                             double&                     nearestSquareDist) const
{
	...
	nearestSquareDist = -1.0;
	{
		...
		for (unsigned i = 0; i < size(); ++i)
		{
			...
			const CCVector3d XP         = (P.toDouble() - X);
