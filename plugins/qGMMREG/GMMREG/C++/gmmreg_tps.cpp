...
int TpsRegistration::PrepareInput(const vnl_matrix<double>& model, const vnl_matrix<double>& scene)
{
	if (Base::PrepareInput(model, scene) < 0)
		return -1;
  return SetCtrlPts(0);
}
...
int TpsRegistration::PrepareOwnOptions(const char* f_config)
{
  ...
  {
	  v_lambda_.resize(4,0);
	  v_affine_.resize(4,0);
  }
