	...
	void importFoliations( ccMainAppInterface *app )
	{
		...
		for (unsigned p = 0; p < cld->size(); p++)
		{
			...
			if (needToApplyRot)
			{
				...
				{
					ccGLMatrix rotX;
					rotX.initFromParameters(-dip * CC_DEG_TO_RAD, CCVector3(1, 0, 0), CCVector3(0, 0, 0)); //plunge
					
					ccGLMatrix rotZ;
					rotZ.initFromParameters(dipdir * CC_DEG_TO_RAD, CCVector3(0, 0, -1), CCVector3(0, 0, 0));
					rotation = rotZ * rotX;
				}
				else //general case
				{
