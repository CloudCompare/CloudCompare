    ...
    , m_doCubic ( doCubic )
{
    ...
    Vector3Tpl<double> axis[4], tr[4];
    double alpha[4];

    for (int i = 0; i < 4; i++)
        vp[i].viewMat.getParameters(alpha[i], axis[i], tr[i]);

    for (int i = 0; i < 4; i++) Y[i] = alpha[i];
    poly_create(m_ts, Y, m_vm_alpha);
    for (int i = 0; i < 4; i++) Y[i] = axis[i].x;
    poly_create(m_ts, Y, m_vm_axis_x);
    for (int i = 0; i < 4; i++) Y[i] = axis[i].y;
    poly_create(m_ts, Y, m_vm_axis_y);
    for (int i = 0; i < 4; i++) Y[i] = axis[i].z;
    poly_create(m_ts, Y, m_vm_axis_z);
...
bool ViewInterpolate::nextView ( cc2DViewportObject& outViewport )
{
	...
	interpView.cameraCenter.z         = poly_interpolate(m_ts, m_cam_cntr_z, interpolate_fraction);
        ...
        /* Interpolation doesn't work like this on a transformation matrix.
         * I know why. I know how to fix it.
         * I just didn't yet...  */

        //Vector3Tpl<double> axis, tr;
        //double alpha;

        //alpha                             = poly_interpolate(m_ts, m_vm_alpha, interpolate_fraction);
        //axis.x                            = poly_interpolate(m_ts, m_vm_axis_x, interpolate_fraction);
        //axis.y                            = poly_interpolate(m_ts, m_vm_axis_y, interpolate_fraction);
        //axis.z                            = poly_interpolate(m_ts, m_vm_axis_z, interpolate_fraction);
        ...
        //axis.normalize();
        //interpView.viewMat.initFromParameters(alpha, axis, tr);
