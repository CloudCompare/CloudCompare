...
void ccMesh::setAssociatedCloud(ccGenericPointCloud* cloud, bool autoRemoveFlags /*=true*/)
{
	if (autoRemoveFlags && nullptr != m_associatedCloud && m_associatedCloud != cloud)
	{
		...
		m_associatedCloud->removeDependencyFlag(this, DP_NOTIFY_OTHER_ON_DELETE);
		m_associatedCloud->removeDependencyFlag(this, DP_NOTIFY_OTHER_ON_UPDATE);
	}
...
void ccMesh::drawMeOnly(CC_DRAW_CONTEXT& context)
{
	...
	if (!lightIsEnabled)
	{
		// if there's no light, no use showing normals
		glParams.showNorms = false;
	}

	// materials & textures
	bool applyMaterials = (hasMaterials() && materialsShown());
	bool uniqueMaterial = (applyMaterials && hasUniqueMaterial());
	...
	{
		// if there's no normals, no need to activate lights
	...
	glFunc->glPushAttrib(GL_LIGHTING_BIT | GL_TRANSFORM_BIT | GL_ENABLE_BIT | GL_TEXTURE_BIT);
	...
	else if (lightIsEnabled && glParams.showNorms)
	{
		// if we don't have any color but we have lights, apply the default material
		ccGL::Color(glFunc, ccColor::whiteRGB); // for textures
		if (context.defaultMat)
		{
			context.defaultMat->applyGL(context.qGLContext, true, false);
		}
	}
	else
	{
		// set the default material diffuse color as the default color
	...
	if (prog)
	{
		...
		if (applyMaterials || showTextures)
		{
			...
			material->applyGL(context.qGLContext, lightIsEnabled, false);
