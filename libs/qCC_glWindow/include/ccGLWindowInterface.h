...
class CCGLWINDOW_LIB_API ccGLWindowInterface : public ccGenericGLDisplay
{
  public:
	...
	**/
	...
	inline bool sunLightEnabled() const
	{
	...
	inline bool customLightEnabled() const
	{
	...
	inline PivotVisibility getPivotVisibility() const
	{
	...
	**/
	...
	inline bool bubbleViewModeEnabled() const
	{
		return m_bubbleViewModeEnabled;
	}
	...
	**/
	...
	inline virtual PICKING_MODE getPickingMode() const
	{
	 ...
	 **/
	inline virtual void lockPickingMode(bool state)
	{
		m_pickingModeLocked = state;
	}

	//! Returns whether picking mode is locked or not
	inline virtual bool isPickingModeLocked() const
	{
		return m_pickingModeLocked;
	}
	...
	**/
	 ...
	 **/
	inline void invalidateVisualization()
	{
	...
	inline const ccGlFilter* getGlFilter() const
	{
		return m_activeGLFilter;
	}

	inline bool areShadersEnabled() const
	{
		return m_shadersEnabled;
	}
	inline bool areGLFiltersEnabled() const
	{
	...
	inline bool hasColorRampShader() const
	{
		return m_colorRampShader != nullptr;
	}

	//! Returns whether rectangular picking is allowed or not
	inline bool isRectangularPickingAllowed() const
	{
		return m_allowRectangularEntityPicking;
	}

	//! Sets whether rectangular picking is allowed or not
	inline void setRectangularPickingAllowed(bool state)
	{
		m_allowRectangularEntityPicking = state;
	}
	...
	inline bool hasOverriddenDisplayParameters() const
	{
		return m_overriddenDisplayParametersEnabled;
	}

	//! Sets picking radius
	inline void setPickingRadius(int radius)
	{
		m_pickRadius = radius;
	}
	//! Returns the current picking radius
	inline int getPickingRadius() const
	{
		return m_pickRadius;
	}
	...
	inline bool scaleIsDisplayed() const
	{
		return m_showScale;
	}

	//! Returns whether the trihedron is displayed or not
	inline bool trihedronIsDisplayed() const
	{
		return m_showTrihedron;
	}
	...
	**/
	...
	inline int glWidth() const
	{
		return m_glViewport.width();
	}
	//! Returns the OpenGL context height
	inline int glHeight() const
	{
		return m_glViewport.height();
	}
	//! Returns the OpenGL context size
	inline QSize glSize() const
	{
		return m_glViewport.size();
	}
  ...
  public: // fullscreen
	...
	inline bool exclusiveFullScreen() const
	{
		return m_exclusiveFullscreen;
	}

  public: // debug traces on screen
	//! Shows debug info on screen
	inline void enableDebugTrace(bool state)
	{
		m_showDebugTraces = state;
	}

	//! Toggles debug info on screen
	inline void toggleDebugTrace()
	{
		m_showDebugTraces = !m_showDebugTraces;
	}
  ...
  public: // stereo mode
	...
	inline bool autoPickPivotAtCenter() const
	{
		return m_autoPickPivotAtCenter;
	}
	...
	inline bool isRotationAxisLocked() const
	{
		return m_rotationAxisLocked;
	}

	//! Returns the default 'vertical' orientation
	inline CCVector3d getDefaultVertDir() const
	{
		return m_rotationAxisLocked ? m_lockedRotationAxis : CCVector3d(0, 0, 1);
	}
  ...
  public: // other methods
	...
	const ccGLWindowSignalEmitter* signalEmitter() const
	{
	...
	void setDisplayScale(const CCVector2d& scale)
	{
		m_displayScale = scale;
	}
	//! Returns the display scale (multiplier)
	const CCVector2d& getDisplayScale() const
	{
  ...
  protected: // rendering
	...
	inline void setGLViewport(int x, int y, int w, int h)
	{
	...
	virtual bool preInitialize(bool& firstTime)
	{
		Q_UNUSED(firstTime);
		return true;
	}
	//! Called at the very end of the initialization process (see initialize)
	virtual bool postInitialize(bool firstTime)
	{
		Q_UNUSED(firstTime);
		return true;
	}
	...
	static void glColor3ubv_safe(QOpenGLFunctions* glFunc, const ccColor::Rgb& color)
	{
		assert(glFunc);
	...
	template <class QOpenGLFunctions>
	static void glColor4ubv_safe(QOpenGLFunctions* glFunc, const ccColor::Rgba& color)
	{
		assert(glFunc);
