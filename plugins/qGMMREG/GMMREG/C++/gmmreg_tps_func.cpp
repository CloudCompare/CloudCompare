...
double ThinPlateSplineFunc_L2::Eval(const double f1, const double f2,
    const vnl_matrix<double>& g1, const vnl_matrix<double>& g2) {
  /* L2 version */
  double f = f1 - 2*f2;
  gradient_ = g1*2 - g2*2;
  return f;
...
double ThinPlateSplineFunc_KC::Eval(const double f1, const double f2,
    const vnl_matrix<double>& g1, const vnl_matrix<double>& g2) {
  /* -KC^2 version */
  double f21 = f2/f1;
  double f =  - f2*f21;
...
void ThinPlateSplineFunc::gradf(const vnl_vector<double>& x,
    vnl_vector<double>& g) {
...
void ThinPlateSplineFunc::PrepareParamGradient() {
