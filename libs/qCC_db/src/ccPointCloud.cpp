	...
	for (unsigned i = 0; i < numberOfPoints; i++)
	{
		...
		if (withNormals)
		{
			...
			case CYLINDER:
			{
				...
				N2.y = -(delta2 - delta);
				N2.z = N.dot(CCVector3::fromArray(axisDir.u));
			}
			break;

			case STRAIGHTENED_CONE:
			{
				double coneAbscissa2 = 0;
				double delta2 = 0;
				double longitude2_rad = 0;
			...
			case CONE:
			{
				...
				ProjectOnCone(AP2, alpha_rad, axisDir, xDir, yDir, coneAbscissa2, delta2, longitude2_rad);
				//unrolling
				double theta2_rad = longitude2_rad * sin_alpha; //sin_alpha is a bit arbitrary here. The aim is mostly to reduce the angular range
				//project the point
				CCVector3d P2out;
		...
		for (; dLongitude_rad < stopAngle_rad; dLongitude_rad += 2 * M_PI)
		{
			...
			case STRAIGHTENED_CONE:
				...
				break;
			case STRAIGHTENED_CONE2:
				Pout += (dLongitude_rad * coneAbscissa * sin_alpha) * xDir2;
				break;

			case CONE:
				Pout += (coneAbscissa * sin(dLongitude_rad)) * xDir2;
				Pout += (-coneAbscissa * cos(dLongitude_rad)) * xDir2;
