...
CC_FILE_ERROR PlyFilter::saveToFile(ccHObject* entity, QString filename, e_ply_storage_mode storageType)
{
...
#else
	p_ply ply = ply_create(qUtf8Printable(filename), storageType, errorCallback, 0, nullptr);
#endif
	if (!ply)
	{
	...
	if (vertices->isA(CC_TYPES::POINT_CLOUD))
	{
		...
		if (sfCount)
		{
			...
			for (unsigned i = 0; i < sfCount; ++i)
			{
				...
				result = ply_add_scalar_property(ply, qUtf8Printable(propName), scalarType);
			}
		}
	}
		...
		if (triNum > 0 && ply_add_element(ply, "face", triNum))
		{
			...
			if (material)
			{
				...
				else
				{
					...
					result = ply_add_comment(ply, qUtf8Printable(QString("TextureFile %1").arg(defaultTextureName)));
	...
	{
		ply_add_comment(ply, qUtf8Printable(FileIO::createdBy()));
		ply_add_comment(ply, qUtf8Printable(FileIO::createdDateTime()));
	}
...
#else
	p_ply ply = ply_open(qUtf8Printable(filename), errorCallback, 0, nullptr);
#endif
	if (!ply)
	{
		return CC_FERR_THIRD_PARTY_LIB_FAILURE;
